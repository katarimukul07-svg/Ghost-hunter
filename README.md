...
The automated player in `tests/automated-player.spec.js` uses the canvas controls to collect a prize and complete the first round. It also checks routes through seeded layouts and verifies a wall collision. Run it alone with `npx playwright test tests/automated-player.spec.js`. On failure, Playwright retains a trace and attaches a JSON replay with the seed, viewport, and recorded moves. The layout and collision fixtures are exposed only when the game loads with `?test=1`.
